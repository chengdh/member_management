...
class pay_type(osv.osv):
    ...
    _name = "member_management.pay_type"
    ...
    def get_pay_type_id(self,cr,uid,pay_type_code):
        '''
        根据pay_type_code获取pay_type_id
        :params string pay_type_code 计费方式编码
        :return integer pay_type_id
        '''
        ids = self.search(cr,uid,[['pay_type_code','=',pay_type_code]])
        if not ids:
            return None
        return ids[0]
